namespace DvirFriedler_DamkaGame
{
    ...
    public class Board
    {
        ...
        internal int GetRow(string i_location)
        {
        ...
        internal int GetCol(string i_location)
        {
        ...
        {
            Console.Clear();
            int i_len_row = this.m_Size + this.m_Size + 2;
            int i_len_col = this.m_Size + 3 * (this.m_Size + 1);
            char[,] i_print_board = new char[i_len_row, i_len_col];

            for (int i = 0; i < i_len_row; i++)
            {
                for (int j = 0; j < i_len_col; j++)
                    i_print_board[i, j] = ' ';
            }

            int i_rowNumber = 0;
            int i_culomnNumber = 0;
            for (int j = 4; j < i_len_col; j += 4)
            {
                i_print_board[0, j] = (char)(65 + i_rowNumber);
                i_rowNumber++;
            }

            for (int i = 1; i < i_len_row; i += 2)
            {
                for (int j = 0; j < i_len_col; j++)
                {
                    i_print_board[i, j] = '=';
            ...
            i_rowNumber = 0;
            for (int i = 2; i < i_len_row; i += 2)
            {
                i_culomnNumber = 0;
                i_print_board[i, 0] = (char)(97 + i_rowNumber);

                for (int j = 2; j < i_len_col; j += 4)
                {
                    i_print_board[i, j] = '|';
                }
                ...
                for (int j = 4; j < i_len_col; j += 4)
                {
                    if (this.m_Board[i_rowNumber, i_culomnNumber] != 0)
                    {
                        i_print_board[i, j] = this.m_Board[i_rowNumber, i_culomnNumber];
                    }
            ...
            {
                for (int j = 0; j < i_len_col; j++)
                    Console.Write(i_print_board[i, j]);
                    Console.Write('\n');
            }
        }
        ...
        internal bool MakeMove(Player io_player, string io_move)
        {
        ...
        internal Piece PieceByPosition(Player io_player, string i_poistion)
        {
            List<Piece> tools;
            if (io_player.m_Tools == 'O')
            {
        ...
        public int CheckifGameOver(Player playerOne, Player playerTwo)
        {
            ...
            if (this.m_Opieces.Count() == 0 || legalmovesO == 0)
            {
